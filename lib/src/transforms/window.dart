...
extension WindowUint8ListExtension<T> on Stream<Uint8List> {
  Stream<Uint8List> window(int size) => _window<Uint8List>(this, size,
      (data, start, stop) => Uint8List.sublistView(data, start, stop));
}

extension WindowInt8ListExtension<T> on Stream<Int8List> {
  Stream<Int8List> window(int size) => _window<Int8List>(this, size,
      (data, start, stop) => Int8List.sublistView(data, start, stop));
}

extension WindowUint16ListExtension<T> on Stream<Uint16List> {
  Stream<Uint16List> window(int size) => _window<Uint16List>(this, size,
      (data, start, stop) => Uint16List.sublistView(data, start, stop));
}
...
extension WindowUint32ListExtension<T> on Stream<Uint32List> {
  Stream<Uint32List> window(int size) => _window<Uint32List>(this, size,
      (data, start, stop) => Uint32List.sublistView(data, start, stop));
}

extension WindowInt32ListExtension<T> on Stream<Int32List> {
  Stream<Int32List> window(int size) => _window<Int32List>(this, size,
